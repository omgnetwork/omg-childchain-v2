defmodule API.ValidatorTest do
  ...
  describe "Basic validation:" do
    ...
    test "hex: positive cases" do
      {:ok, hex_1_value} = @params |> Map.get("hex_1") |> from_hex()
      assert {:ok, hex_1_value} == expect(@params, "hex_1", :hex)

      {:ok, hex_2_value} = @params |> Map.get("hex_2") |> from_hex()
      assert {:ok, hex_2_value} == expect(@params, "hex_2", :hex)

      {:ok, hex_3_value} = @params |> Map.get("hex_3") |> from_hex()
  ...
  describe "list and map preprocessing:" do
    ...
    test "parsing map" do
      ...
      {:ok, address_value} = @params |> Map.get("valid_address") |> from_hex()
  ...
  describe "Preprocessors:" do
    ...
    test "signature should validate both hex value and length" do
      {:ok, signature_value} = @params |> Map.get("valid_signature") |> from_hex()
    ...
    test "hash should validate both hex value and length" do
      {:ok, hash_value} = @params |> Map.get("valid_hash") |> from_hex()
