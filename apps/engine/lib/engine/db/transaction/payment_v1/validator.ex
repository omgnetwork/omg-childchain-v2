defmodule Engine.DB.Transaction.PaymentV1.Validator do
  ...
  :ok
  """
  @spec validate(Type.output_list_t(), Type.output_list_t(), Type.accepted_fees_t()) :: Type.validation_result_t()
  @impl Engine.DB.Transaction.Validator
  def validate(input_data, output_data, fees) do
    fees
    |> validate_merge_fees(input_data, output_data)
    |> AmountValidator.validate(input_data, output_data)
  end
