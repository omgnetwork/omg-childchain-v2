    ...
    for repo <- repos do
      spawn_link(fn ->
        ...
        case System.get_env("DATABASE_URL") do
          ...
          url ->
            Logger.info("Setting DB url from DATABASE_URL env var.")
            repo_config = Application.get_env(:engine, repo)
            _ = Application.put_env(@app, repo, Keyword.merge(repo_config, url: url))
            Logger.info("Updated Engine #{repo} config: #{inspect(Application.get_env(:engine, repo))}")
        end
