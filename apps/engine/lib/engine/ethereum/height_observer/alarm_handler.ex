defmodule Engine.Ethereum.HeightObserver.AlarmHandler do
  ...
  def handle_event({:set_alarm, {:ethereum_connection_error, %{reporter: consumer}}}, %{consumer: consumer} = state) do
    _ = Logger.warn(":ethereum_connection_error alarm raised.")
    :ok = GenServer.cast(state.consumer, {:set_alarm, :ethereum_connection_error})
    {:ok, state}
  end

  def handle_event({:clear_alarm, {:ethereum_connection_error, %{reporter: consumer}}}, %{consumer: consumer} = state) do
  ...
  def handle_event({:set_alarm, {:ethereum_stalled_sync, %{reporter: consumer}}}, %{consumer: consumer} = state) do
    _ = Logger.warn(":ethereum_stalled_sync alarm raised.")
    :ok = GenServer.cast(state.consumer, {:set_alarm, :ethereum_stalled_sync})
    {:ok, state}
  end

  def handle_event({:clear_alarm, {:ethereum_stalled_sync, %{reporter: consumer}}}, %{consumer: consumer} = state) do
